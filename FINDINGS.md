...
=> per-ref token 区间: ref 段 [4144, 8288), 网格 74x56, 行优先。
   ...
   实验 2 的 mask 坐标基础已具备。
## 21. 实验 0 结论: 指令可覆盖 canvas 位置 (S2 降级)
canvas 固定猴左女右, P1 要求猴左 / P2 要求女左, 各 5 seed。
**结果 10/10 全部遵循指令**, P2 五张全部女孩在左。
=> 用语义描述 ("the monkey"/"the girl") 指定各自去向完全可靠,
   canvas 位置先验不构成约束。
=> S2 (无 per-reference 可寻址) 降级为接口观察, **不是失败模式**, 撑不起 motivation。

### 但暴露两个待验证的真实现象
(a) source 残留: 5/10 图中可见皮卡丘的黄色碎片 (尾巴/耳朵/身体),
    粘在角色身上或飘在旁边。原主体未被干净替换。
(b) reference 细节丢失: ref1 的法杖仅 3/10 保留; 两角色五官在多数图中退化为色块。
    法杖在背影中本可保留 (P2-s2/s3 证明), 却多数丢失
    => 不是姿态问题, 是 ref 细节未被传递。

下一步: (a)(b) 均比位置绑定更值得量化。先做不预设假设的失败普查, 确定真实失败分布。
